...
with st.sidebar:
    selection = st.radio('Select what you want to do', ('Know what an image is about', 'Identify objects in an image'))

if selection == 'Know what an image is about':
    ...
    if picture is not None:
        if save_uploaded_image(picture):
            ...
            result = classify(str(os.path.join('uploads', picture.name)))
            st.text('\nOR\n'.join([i['label'] for i in result]))
        else:
            st.write('Image did not upload properly')

if selection == 'Identify objects in an image':
